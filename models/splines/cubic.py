...
def cubic_spline(inputs,
    ...
    if inverse:
        bin_idx = helper_functions.searchsorted(cumheights, inputs)[..., None]
    else:
        bin_idx = helper_functions.searchsorted(cumwidths, inputs)[..., None]
    ...
    if inverse:
        ...
        p = helper_functions.cbrt((-depressed_1[one_root_mask] + torch.sqrt(-discriminant[one_root_mask])) / 2.)
        q = helper_functions.cbrt((-depressed_1[one_root_mask] - torch.sqrt(-discriminant[one_root_mask])) / 2.)
